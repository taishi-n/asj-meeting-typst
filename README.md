...
[`main.typ`をコンパイルしたPDF（全3ページ）](assets/main.pdf)を確認できます．
PDFは`main`ブランチへのpush時にGitHub Actionsで再生成されます．
以下は1ページ目のプレビューです．

![サンプル原稿の1ページ目](assets/preview.png)

## 特長

- A4判，本文幅165 mm，二段組，段間7 mmの紙面を設定します．
- 本文文字サイズは10 ptと11 ptから選択できます．
...
| `body-size` | `length` | `10pt` | 本文文字サイズ．`10pt`または`11pt` |

`body-size`へ指定できる値は`10pt`と`11pt`だけです．
既定値は`10pt`であり，`11pt`へ変更することもできます．
